...
To find more RNA dependent proteins, we will include a 3rd criteria to classify the protein as RNA dependent. The 3rd criteria is the result of t-test of local maxima. The reason is that from 2088 Proteins that are listed in the data bank, we only identify 228 of 2088 proteins as RNA dependent protein with 2 conditions. With a 3rd condition, we expect more proteins to be identified as RNA dependent. 
...
The QQ plot shows us that the residuals are normally distributed with heavy tails. The normal distribution of residues is centered at 0, that the model fits the real values well. The plot of predicted and actual value of Y shift shows us, that the model doesn't really work when the values are below 0. When the points are closer to the line, it indicates that the predicted values are close to the actual one. Nevertheless, the model works decently when the values are above 0. Sadly, the actual Y shift has a very high spread and makes it hard to find a perfect linear model to predit y shift. 
...
The QQ plot shows us that the residuals are normally distributed with heavy tails. The normal distribution of residues is centered at 0, that the model fits the real values well. The plot of predicted and actual value of X shift works fairly good when the x shift are very minimal (shift by one fraction). The linear model unfortunately doesn't predict very well due to the variability of the x shift, especially when the shift is higher. 