...
Rep_1_norm_Ctrl_sums = rowSums(Rep_1_norm_RNase)
Rep_1_norm_RNase_sums
Rep_2_norm_RNase_sums = rowSums(Rep_2_norm_RNase)
Rep_2_norm_RNase_sums
Rep_3_norm_RNase_sums = rowSums(Rep_3_norm_RNase)
...
MICA_HUMAN_Protein_RNase_Norm_1 = ggplot() + 
  geom_line(data = select(Rep_1_norm_RNase, contains("MICA_HUMAN_Rep_1")),  aes(x = Fractions, y = MICA_HUMAN_Rep_1, color= 'Ctrl_Rep1')) + 
  
  geom_line(data = select(Rep_2_norm_RNase, contains("MICA_HUMAN_Rep_2")),  aes(x = Fractions, y = MICA_HUMAN_Rep_2 , color= 'Ctrl_Rep2')) + 
  
  geom_line(data = select(Rep_3_norm_RNase, contains("MICA_HUMAN_Rep_3")),  aes(x = Fractions, y = MICA_HUMAN_Rep_3 , color= 'Ctrl_Rep3')) +
...
colnames(new_df_anti_outliers) <- colnames(Norm_RNase)
...
Ctrl_anti_outlier_df = anti_outlier_function(Norm_Ctrl)
RNase_anti_outlier_df = anti_outlier_function(Norm_RNase)
...
rownames(new_df_combi) <- rownames(Norm_Ctrl)
colnames(new_df_combi) <- colnames(Norm_RNase)
...
Ctrl_combi_df = df_combi_function(Norm_Ctrl)
RNase_combi_df = df_combi_function(Norm_RNase)
Ctrl_combi_df
...
maxnum_plot = function(input, row){
  ...
  else if (all(input == maxnum_RNase_0.1)){
  output = 0
  output[1] = input[row]
  for (i in seq(0.2,0.9,0.1)){
...
#By Madleen 
##Richard's code, with a couple embellishments

maxnum_plot_col = function(input, row){
  if (all(input == maxnum_Ctrl_0.1)){
  output = 0
  output[1] = input[row]
  for (i in seq(0.2,0.9,0.1)){
  ...
  plot(seq(0.1,0.9,0.1),col = c(rand_color(n=1)),output, main = "Ctrl Protein", xlab="Threshold", ylab="Maxima Number", type="l", lwd=5)
  }
  ...
  plot(seq(0.1,0.9,0.1),col = c(rand_color(n=1)) ,output, main = "RNase Protein", xlab="Threshold", ylab="Maxima Number", type="l", lwd=5)
  }
  
  else {print("Invalid Input")}
}

maxnum_plot_col(maxnum_Ctrl_0.1, 10)
maxnum_plot_col(maxnum_RNase_0.1, 10)
...
```{r}
# Function to make data frame where absolute maxima of 3 Reps of a protein are inserted in a column (Threshold hasn't been determined)
...
abmax_Ctrl = abmax(maxima_Ctrl_0.3)
abmax_RNase = abmax(maxima_RNase_0.3)

# T-Test comparing each corresponding Protein Rep of Ctrl and RNase (need to adjust the alpha value through Multiple Tests: Bonferroni Correction?)
...
test = ttest(abmax_Ctrl, abmax_RNase, 0.05/2)
test
```
