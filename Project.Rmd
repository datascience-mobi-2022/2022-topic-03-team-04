...
Our first aim is to restructure the data set by fractions and divide it in two data sets, one for RNase and one for Ctrl, which will make the further analysis easier to perform. Therefor we create a restructure fucntion.
...
We compute the sums fractionwise and find the two closest sums in order to define normalization factors for the replicates. The quotients of the mean sums and sums of replicates are the noramlization factors.
...
# In order that function works the normalized Datasets have to be reordered
...
T Test shows us if the global maxima of each of the proteins (all repetitions) between Ctrl and RNase group has a significance difference. A significant difference between the two groups will be categorized as RNA dependent and it will be our first criteria for RNA dependency.
...
By comparing our findings with the RBP lists databank we calculated the true postitive, true negative, false positive and false negative rates. The results can be seen in the table. 
...
Ctrl and RNase should have lower correlation for RNA dependent than RNA indepedent. We use the correlation between Ctrl and RNase to predict the Y Shift.
...
The QQ plot shows us that the residuals are normally distributed with heavy tails. The normal distribution of residues is centered at 0, that the model fits the real values well. The plot of predicted and actual values of Y shifts shows us, that the model doesn't really work when the values are below 0. When the points are closer to the line, it indicates that the predicted values are close to the actual one. Nevertheless, the model works decently when the values are above 0. Sadly, the actual Y shift has a very high spread and makes it hard to find a perfect linear model to predict the Y shift. 


## 8.2 Linear Regression using RBP2GO_Score to Predict X Shift


After testing multiplte variables from the new data bank and the data that we have, we decide to use these 2 variables because the other variables  have low correlation and our variables have p value that is less than 0.05 in linear regression. 
...
The QQ plot shows us that the residuals are normally distributed with heavy tails. The normal distribution of residues is centered at 0, that the model fits the real values well. The plot of predicted and actual values of X shifts works fairly good when the X shifts are very minimal (shift by one fraction). The linear model unfortunately doesn't predict very well due to the variability of the X shifts, especially when the shifts are higher. 