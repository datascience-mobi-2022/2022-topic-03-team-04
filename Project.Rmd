...
Data.norm.ctrl <- as.data.frame(scale(Ctrl[1:11040]))
head(Data.norm.ctrl)
Data.norm.RNase <- as.data.frame(scale(RNase[1:11040]))
...
Data.norm.ctrl.minmax <- as.data.frame(apply(Ctrl,2,function(x){
    (x - min(x)) / (max(x) - min(x))
  }))
head(Data.norm.ctrl)
Data.norm.RNase.minmax <- as.data.frame(apply(RNase,2,function(x){
    (x - min(x)) / (max(x) - min(x))
  }))
head(Data.norm.RNase)
