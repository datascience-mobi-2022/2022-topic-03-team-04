...
```{r}
# Maxima Detection (Next Step: Determine which are included as Local Maxima)
maximafunction = function(input){
  ...
  if (input[i+1]-input[i]>0) {
    ...
    c = sort(input[which(diff(b)==-1)+1], decreasing = TRUE)
    # 1:10 = First until Tenth Maxima
    for (j in 1:10){ 
      # Direct below we can add threshold using if to classify which will be included as peaks 
      output[j] = c[j]  
    }
  }
  return(output)
...
allmaxima_Ctrl = data.frame(apply(tCtrl_Norm, 2, maximafunction))
allmaxima_Ctrl = data.frame(apply(tRNase_Norm, 2, maximafunction))
# Rows represent 1st-10th Maxima
```
