...
###Additional edit by Richard to make the code more compact (2 graphs in a plot)
...
maxnum_plot_col()
# After seeing some proteins, it seems Threshold 0.5 is the best, where the number of maxima doesn't exceed 3
...
abmax_Ctrl = abmax(maxima_Ctrl_0.5)
abmax_RNase = abmax(maxima_RNase_0.5)
...
# X Shift
rowmax_Ctrl = apply(maxima_Ctrl_0.5, 2, which.max)
rowmax_RNase = apply(maxima_RNase_0.5, 2, which.max)
...
# Combine Y-Shift, X-Shift and P Value in a Data Frame
pxy = data.frame(P_Value = p_value, X_Shift = x_shift, Y_Shift = y_shift)
pxy = pxy[-which(is.na(pxy)),]

# Elbow Method
wss = sapply(2:8,function(x){ 
  kmeans(pxy, x)$tot.withinss
})
...
library(cluster)
library(factoextra)
fviz_cluster(kmeans(pxy, 3), data = pxy, geom = c("point"), main = "K Means Clustering with 3 Clusters")
