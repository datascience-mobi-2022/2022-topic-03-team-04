...
##--> method divides each value by the feature's maximum value ---> result: values range from 0 to 1

#2. Min-Max
##takes each value, substracts X old from the features minimal value and divides it by the fearures range 
##the new values are in the range of 0 to 1

#3. Z-Score
##standard score --> remove Mu, which is the features average, from each value before dividing it by the standard deviation 
...
Data.norm.Ctrl <- as.data.frame(apply(Ctrl,2,function(x){(x-mean(x))/sd(x)}))
head(Data.norm.ctrl)
Data.norm.RNase <- as.data.frame(apply(RNase,2,function(x){(x-mean(x))/sd(x)}))
head(Data.norm.RNase)
...
Fractions = c(1:25)
SPB6_Human_Protein_Ctrl_norm = ggplot() + 
  geom_line(data = select(Data.norm.ctrl,1),  aes(x = Fractions, y = SPB6_HUMAN_Rep_1, color= 'Ctrl_Rep1')) + 
  
  geom_line(data = select(Data.norm.ctrl,2),  aes(x = Fractions, y = SPB6_HUMAN_Rep_2 , color= 'Ctrl_Rep2')) + 
  
  geom_line(data = select(Data.norm.ctrl,3),  aes(x = Fractions, y = SPB6_HUMAN_Rep_3 , color= 'Ctrl_Rep3')) +
  
  scale_colour_manual(name="legend",values=c(Ctrl_Rep1="blue2",Ctrl_Rep2="cyan3", Ctrl_Rep3="violetred4")) +
  
  labs(title="SPB6_HUMAN_Ctrl_Normalized", y= "Protein Amount") 
 SPB6_Human_Protein_Ctrl_norm + scale_x_continuous(breaks = seq(1, 25, by = 1))
  
  
SPB6_Human_Protein_RNase_norm = ggplot() + 
  geom_line(data = select(Data.norm.RNase,1),  aes(x = Fractions, y = SPB6_HUMAN_Rep_1 , color= 'RNase_Rep1')) + 
  
  geom_line(data = select(Data.norm.RNase,2), aes(x = Fractions, y = SPB6_HUMAN_Rep_2, color= 'RNase_Rep2')) +
  
  geom_line(data = select(Data.norm.RNase,3), aes(x = Fractions, y = SPB6_HUMAN_Rep_3, color= 'RNase_Rep3')) +
  
   scale_colour_manual(name="legend",values=c(RNase_Rep1="blue2",RNase_Rep2="cyan3", RNase_Rep3= "violetred4")) +
  
  labs(title="SPB6_HUMAN_RNase_Normalized", y= "Protein Amount") 
  SPB6_Human_Protein_RNase_norm + scale_x_continuous(breaks = seq(1, 25, by = 1))
