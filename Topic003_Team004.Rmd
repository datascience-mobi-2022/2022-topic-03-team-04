...
author: "Khalida Dushimova, Madleen Piegsa, Greta Karathanos and Richard Efraim Langi"
date: "4/27/2022"
...
Greta: Normalization, Report, Presentation
Khalida: Coding (analysis), Report, Presentation 
Madleen: Results visualization, Presentation
Richard: Coding (analysis), Report, Presentation


## 1. Description of the data set (Raw data)


```{r 1.1 loading data}

Data.raw <- read.table("data/RDeeP_A549_NS.csv", header=TRUE, row.names=1, sep = ";")
...
Our first aim is to restructure the data set by fractions and divide it into two data sets, one for RNase and one for Ctrl, which will make further analysis easier to perform. Therefore we create a restructure fucntion.
...
restructure_func = function(input_df,start){ #start = 1 for Ctrl and start = 2 for RNase.
  ...
  while (i <= dim(output_df)[1]) { # All Rows of the new dataframes will be selected
    ...
    k = k + 1 # The row of the old data frame is advanced by 1
  }
  return(output_df)
}
...
MICA_HUMAN_Protein  = ggplot() +
 geom_line(data = select(Ctrl, contains("MICA_HUMAN_Rep_1")),  aes(x = Fractions, y = MICA_HUMAN_Rep_1, color= 'Ctrl_Rep1')) + 
  ...
  scale_colour_manual(name="legend",values=c(Ctrl_Rep1="blue2",RNase_Rep1="red", Ctrl_Rep2="magenta1", RNase_Rep2="skyblue",Ctrl_Rep3="darkviolet", RNase_Rep3="turquoise4")) +
...
MICA_HUMAN_Protein_Ctrl = ggplot() + 
  ...
  scale_colour_manual(name="legend",values=c(Ctrl_Rep1="green",Ctrl_Rep2="purple4", Ctrl_Rep3="magenta1")) +
...
MICA_Human_Protein_RNase = ggplot() + 
  ...
  geom_line(data = select(RNase,contains("MICA_HUMAN_Rep_3")), aes(x = Fractions, y = MICA_HUMAN_Rep_3, color= 'RNase_Rep3')) +
   ...
   scale_colour_manual(name="legend",values=c(RNase_Rep1="blue2",RNase_Rep2="red", RNase_Rep3= "magenta1")) +
...
We compute the sums fractionwise and find the two closest sums to define normalization factors for the replicates. The quotients of the mean sums and sums of replicates are the normalization factors.
...
# So that function works the normalized Datasets have to be reordered
...
saveRDS(Norm_Ctrl, file = "data/Norm_Ctrl.rds")

saveRDS(Norm_RNase, file = "data/Norm_RNase.rds")

Data_Norm_RNase <- readRDS("data/Norm_RNase.rds")

Data_Norm_Ctrl <- readRDS("data/Norm_Ctrl.rds")
```


```{r}

#aim is to eliminate outliers in our dataset, we try to create our function
...
MICA_HUMAN_Protein_Ctrl = ggplot() + 
  ...
  scale_colour_manual(name="legend",values=c(Ctrl_Rep1="blue2",Ctrl_Rep2="red", Ctrl_Rep3="black")) +
...
MICA_HUMAN_Protein_Ctrl_Norm_Second = ggplot() + 
  ...
  scale_colour_manual(name="legend",values=c(Ctrl_Rep1="red",Ctrl_Rep2="black", Ctrl_Rep3="#6600FF")) +
...
The method is based on checking the neighbours of each fraction. For the first fraction, only the right neighbour will be compared because there is no left neighbour. For the second-twenty-fourth fraction, both left and right neighbours will be compared. For the last fraction, only the left neighbour will be compared since right neighbour doesn't exist. If the checked fraction has higher values than the neighbour(s), then the fraction is a maxima. If the the checked fraction has lower values than the neighbour(s), then the fraction is a not a maxima and will be zero-ed in our code.
...
# Function to plot random protein with thresholds on the x-axis and number of maxima on the y-axis (only correctly works with input either maxnum_Ctrl_0.1 or "maxnum_RNase_0.1", nonetheless with any desired rows
...
# Function to get a list with only local maxima (the local maxima of all 3 reps are combined in Rep1. Rep2 and Rep3 are useless)
...
T Test shows us if the global maxima of each of the proteins (all repetitions) between Ctrl and RNase group has a significant difference. A significant difference between the two groups will be categorized as RNA dependent and it will be our first criterion for RNA dependency.

H0: There is no significant difference between RNase and Ctrl (p-value > alpha)
H1: There is a significant difference between RNase and Ctrl (p-value < alpha)
...
Through K-Means Clustering and comparing our t-test results, we define selection criteria (Y Shift and X Shift) for determining RNA-dependent Proteins. Y Shift is the difference of protein amount between the global maxima of Ctrl and RNase Group. X Shift is the difference of locations of global maxima (fraction) between the global maxima of Ctrl and RNase Group. If the X and Y Shift has positive values (left and down Shift), we define the protein as RNA dependent. If the X and Y Shift has negative values or is close to zero (right and up Shift), we define the protein as RNA independent. These shifts will be our second criteria for RNA dependency.
...
"Match" shows 1 (True) and 0 (False). True means that the results from the T-Test in terms of RNA (in)dependency are the same as K-Means and the other way around for False.
...
We have defined 2 Criteria for RNA dependency. The first is the result from the T-Test where a significant difference in absolute maxima means RNA dependency. The second one is the result from K means clustering where proteins are grouped into three clusters by their X- and Y-shift characteristics. Positive shift (RNA dependency), no shift (not defined dependency) and negative shift (no RNA dependency). Below we consider 2 cases. First when both criteria are fulfilled and the other one when either criterion is satisfied.
...
# Data Frames for each RNA dependent and independent protein (Both Conditions Fulfilled)
...
# Data Frames for each RNA dependent and independent protein (Either Conditions Fulfilled)
...
## 6.2 Comparing with Data Bank (RBP Lists)
All RNA dependent proteins from the data bank were used including the ones that has 0 mentions in Times_listed_as_RBP.

```{r}
# Data Bank with All RNA Dependent Proteins
...
# Take only proteins that are at least one time identified from the data bank
...
Too many RNA dependent proteins were not identified (2017), so we have to try to find more maxima and more potential dependent proteins, we can apply our criteria on 
--> local maxima t-test and applying our criteria on detected values
...
To find more RNA dependent proteins, we will include a 3rd criteria to classify the protein as RNA dependent. The 3rd criteria is the result of the T-Test of local maxima. Of 2250 Proteins that are listed in the data bank, we only identify 233 of 2017 proteins as RNA dependent with 2 conditions. With a 3rd condition, we expect more proteins to be identified as RNA dependent. 
...
By comparing our findings with the RBP lists databank we calculated the true positive, true negative, false positive and false negative rates. The results can be seen in the table. 
...
Ctrl and RNase should have a lower correlation for RNA dependent than RNA independent. We use the correlation between Ctrl and RNase to predict the Y Shift.
...
The QQ plot shows us that the residuals are normally distributed with heavy tails. The normal distribution of residues is centered at 0, so the model fits the real values well. The plot of predicted and actual values of Y shifts shows us, that the model doesn't really work when the values are below 0. When the points are closer to the line, it indicates that the predicted values are close to the actual ones. Nevertheless, the model works decently when the values are above 0. Sadly, the actual Y shift has a very high spread and makes it hard to find a perfect linear model to predict the Y shift. 


## 8.2 Linear Regression using RBP2GO_Score to Predict X Shift


After testing multiple variables from the new data bank and the data that we have, we decide to use these 2 variables because the other variables have low correlation and our variables have a p-value that is less than 0.05 in linear regression. 
...
The QQ plot shows us that the residuals are normally distributed with heavy tails. The normal distribution of residues is centered at 0, so model fits the real values well. The plot of predicted and actual values of X shifts works fairly good when the X shifts are very minimal (shift by one fraction). The linear model unfortunately doesn't predict very well due to the variability of the X shifts, especially when the shifts are higher. 